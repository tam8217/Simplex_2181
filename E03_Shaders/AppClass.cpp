...
{
	m_uShaderProgramID = LoadShaders("Shaders//BasicColor.vs", "Shaders//ChangingColor.fs");
...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	...
	//Changing the complimentary colors to be on or off
...
void AppClass::Display(void)
{
	...
	//Enabling or disabling the complimentary 
	if (compColors)
	{
		GLuint CompliColorsEnabled = glGetUniformLocation(m_uShaderProgramID, "CompliColorsEnabled");
		glUniform1f(CompliColorsEnabled, 1.0f);
	}
	else
	{
		GLuint CompliColorsEnabled = glGetUniformLocation(m_uShaderProgramID, "CompliColorsEnabled");
		glUniform1f(CompliColorsEnabled, 0.0f);
	}
